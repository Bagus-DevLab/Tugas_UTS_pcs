        ...
        <>
            ...
            <div className="mx-auto max-w-5xl space-y-6 p-4">
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold" style={{ color: palette.deep }}>Detail Deteksi #{detection.id}</h1>
                ...
                {detection.user && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                        <Card style={{ borderColor: palette.teal }}>
                            <CardContent className="flex items-center gap-4 p-4">
                                <div className="flex h-10 w-10 items-center justify-center rounded-full" style={{ backgroundColor: `${palette.teal}20` }}>
                                    <UserIcon className="h-5 w-5" style={{ color: palette.teal }} />
                                </div>
                                <div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    {detection.image_path && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="md:row-span-2">
                            <Card>
                                <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><ImageIcon className="h-4 w-4" style={{ color: palette.teal }} /> Citra Daun</CardTitle></CardHeader>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                        <Card>
                            <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><Activity className="h-4 w-4" style={{ color: palette.teal }} /> Label Penyakit</CardTitle></CardHeader>
                            <CardContent>
                                <p className="text-xl font-bold" style={{ color: palette.deep }}>{detection.label ?? '-'}</p>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                        <Card>
                            ...
                            <CardContent>
                                <p className="text-3xl font-bold" style={{ color: palette.deep }}>{confidence !== null ? `${confidence}%` : '-'}</p>
                                {confidence !== null && (
                                    <div className="mt-2 h-2.5 overflow-hidden rounded-full" style={{ backgroundColor: `${palette.sand}40` }}>
                                        <motion.div className="h-full rounded-full" style={{ backgroundColor: confidence >= 80 ? palette.sage : confidence >= 50 ? palette.teal : '#c45c5c' }} initial={{ width: 0 }} animate={{ width: `${confidence}%` }} transition={{ duration: 0.8 }} />
                                    </div>
                                )}
                            </CardContent>
                        </Card>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                        <Card>
                            <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><Thermometer className="h-4 w-4" style={{ color: palette.teal }} /> Suhu</CardTitle></CardHeader>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
                        <Card>
                            <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><Clock className="h-4 w-4" style={{ color: palette.teal }} /> Waktu Pemindaian</CardTitle></CardHeader>
                            <CardContent>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                        <Card>
                            <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><MapPin className="h-4 w-4" style={{ color: palette.teal }} /> Titik Koordinat</CardTitle></CardHeader>
                            <CardContent>
                                {detection.latitude !== null && detection.longitude !== null ? (
                                    <div>
                                        ...
                                        <a href={`https://www.google.com/maps?q=${detection.latitude},${detection.longitude}`} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs hover:underline" style={{ color: palette.teal }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}>
                        <Card>
                            <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm">{detection.connection_status === 'online' ? <Wifi className="h-4 w-4" style={{ color: palette.sage }} /> : <WifiOff className="h-4 w-4 text-red-500" />} Status Koneksi</CardTitle></CardHeader>
                            <CardContent>
                                <Badge style={detection.connection_status === 'online' ? { backgroundColor: `${palette.sage}20`, color: palette.deep } : { backgroundColor: '#fee2e2', color: '#dc2626' }}>
                    ...
                    {detection.disease?.treatments && detection.disease.treatments.length > 0 && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="md:col-span-2 lg:col-span-3">
                            <Card>
                                <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm"><FlaskConical className="h-4 w-4" style={{ color: palette.teal }} /> Rekomendasi Tindakan & Dosis</CardTitle></CardHeader>
                                <CardContent>
                                    <div className="space-y-2">
                                        {[...detection.disease.treatments].sort((a, b) => a.priority - b.priority).map((t) => (
                                            <div key={t.id} className="flex items-start gap-3 rounded-lg border p-3" style={{ borderColor: palette.sand }}>
                                                <Badge variant="outline" className="shrink-0 text-[10px]" style={{ borderColor: palette.teal, color: palette.deep }}>{getTreatmentTypeLabel(t.type)}</Badge>
                                    ...
                                    <div className="space-y-2">
                                        {Object.entries(detection.predictions).sort(([, a], [, b]) => (b as number) - (a as number)).map(([label, conf]) => (
                                            <div key={label} className="flex items-center gap-3 text-sm">
                                                ...
                                                <div className="h-2 flex-1 overflow-hidden rounded-full" style={{ backgroundColor: `${palette.sand}40` }}>
                                                    <div className="h-full rounded-full" style={{ width: `${conf}%`, backgroundColor: palette.teal }} />
                                                </div>
